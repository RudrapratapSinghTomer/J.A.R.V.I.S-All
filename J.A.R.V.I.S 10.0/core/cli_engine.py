...
class CLIEngine:
    ...
    def _request_self_correction(self, command: str, error_msg: str) -> dict:
        ...
        prompt = (
            ...
            "3. Shell Syntax & Encodings: Ensure shell quoting and file paths match the environment rules (Windows host vs bash guest). On Windows, all file paths containing spaces MUST be fully wrapped in double quotes (e.g., '\"C:\\Users\\developer\\Desktop\\J.A.R.V.I.S All\\...\"') to prevent them from being split into multiple arguments by cmd or powershell.\n"
            "4. Pip Executables on Windows PATH: On Windows environments, pip-installed CLI executables (like 'bandit', 'pytest', etc.) are often placed in python's Scripts directory which might not be on the active system PATH. When running or correcting pip-installed tools, ALWAYS run them as 'python -m <tool_name>' (e.g., 'python -m bandit' instead of 'bandit') to guarantee they execute successfully.\n"
            "5. Destructive Commands Strictly Forbidden: You are strictly FORBIDDEN from generating destructive commands that delete, remove, or destroy existing files or directories (such as 'rmdir /s /q', 'rm -rf', or 'del') in either 'pre_commands' or 'fixed_command'. NEVER attempt to solve an error by deleting existing project files or directories.\n\n"
