...
class TrueBackendThinkingEngine:
    ...
    def analyze_and_plan(self, query: str, system_context: str, past_episodes: List[dict]) -> dict:
        ...
        prompt = (
            ...
            "CRITICAL META-PLANNING CONSTRAINT:\n"
            "If the user query asks to 'make a plan', 'create a plan', or 'draft a plan' and specifies a target OUTPUT path or directory (e.g. 'OUTPUT: create a plan at <path>'), the synthesized plan steps MUST focus solely on generating and writing the final plan document (e.g. as a JSON or markdown file) to that specified path. The steps must be limited to creating, writing, and validating the plan file itself. You must NOT generate plan steps that attempt to execute the actions described in the plan (such as performing actual code refactoring, system changes, or memory updates) during this execution. You already possess the codebase context in your prompt to draft the plan.\n"
            ...
            "        \"command_or_action\": \"The exact, concrete executable action to perform. If type=terminal, this MUST be a valid, executable shell command or python script (e.g. 'python tests/test_memory.py' or 'mkdir plans'). On Windows, all file paths containing spaces MUST be fully wrapped in double quotes (e.g. 'mkdir \"C:\\path to\\folder\"'). If type=edit, this must be a script or tool command to create/modify a file. If type=vision, use 'capture_and_analyze <prompt>' or 'analyze_existing <image_path> <prompt>'. If type=browser, use 'search <term>' or 'navigate to <url>'\",\n"
            "        \"dependencies\": []\n"
            "      }\n"
            "    ]\n"
            "  },\n"
